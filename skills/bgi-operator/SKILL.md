...
3. `folderName` 已被现有配置组引用时可视为已安装；新引入的路线或脚本才需要额外验证。
4. JS 任务必须核对脚本参数定义；没有可靠默认值的必填参数才构成用户缺项。
5. 计算顺序字段时复用目录投影里的 `index`；不要再逐文件读取文件头。
6. 调用 `bgi.user.write` 提交完整文件。替换已有文件时传 `expectedSha256`；新建时省略。运行时审批就是写入确认，不在对话里重复询问。
...
5. 运行时审批负责确认写操作。提交后回读；保存 `changeId` 供回退。
6. 候选不可写或语义不确定时停止修改并说明具体缺口，不靠字段名猜效果。

## 执行任务

1. 运行现有配置组时，用一次 `bgi.user.list` 对 `ScriptGroup` 投影 `name,index,projects`，由组名或任务内容解析目标。组已存在时不再列 User 根目录、AutoPathing 或 JsScript 验证组内引用；创建或修改资源时才验证引用。
2. 只有准备提交执行时才调用一次 `bgi.state.get`，检查截图器、游戏句柄、任务锁和窗口状态。纯查询或文件编辑不需要状态快照。
3. 运行调度器配置组时直接读取 `bgi.run_script_group` 契约，并传 User/ScriptGroup 中的精确 `name`；不搜索低层 `script_control` 命令，也不要求用户预先在界面选中。
4. 其他动作若已知道精确 `methodId`，直接 `bgi.api.describe`；否则只在 `command` 组按一个动作词搜索一次。
5. 契约必须同时满足：`callable=true`、参数可提供、接口确实作用于目标对象。其他低层命令仍依赖界面当前选择时，不得声称能按名称执行。
6. 调用 `bgi.api.invoke` 后用返回的 Job ID 查询到终态。完成只证明处理器返回；按契约要求复查状态或结果。
7. 接口不可调用时直接说明唯一阻塞项。不要继续换中英文、查插件、搜生命周期接口或让用户重复提供已经查到的信息。
...
不要询问 BetterGI 是否运行、目录在哪里、有哪些配置组、接口是否可用、是否需要保存、是否允许执行已明确要求的写操作。程序会获取这些事实并在真正写入前显示审批。
