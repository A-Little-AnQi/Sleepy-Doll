...
/// <summary>Business explanations checked against host configuration consumers and UI bindings.</summary>
public static class SettingDocumentation
{
    ...
    {
        ["AutoBossConfig.BossName"] = "自动讨伐的首领名称；从宿主支持的首领列表选择，修改名称不会立即开始讨伐。",
        ...
        ["AutoFightConfig.StrategyName"] = "自动战斗使用的策略名称；按宿主当前可用策略选择，不编造文件名。配置本身不启动战斗。",
        ...
        ["NotificationConfig.NotificationEventSubscribe"] = "通知事件订阅列表，用于筛选哪些宿主事件触发通知；渠道与接收方另由各通知渠道配置决定。",
    ...
    public static string? Find(Type owner, PropertyInfo property)
    {
        ...
        if (property.Name == "HasErrors") return "宿主配置验证器是否存在校验错误；这是只读诊断状态，不是可修改的开关。";
