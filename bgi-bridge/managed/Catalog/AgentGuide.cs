...
public static class AgentGuides
{
    public static AgentGuide For(string id)
    {
        var (title, purpose, when, result, verify, rollback, example) = id switch
        {
            ...
            "bgi.probe" => ("宿主兼容性", "检查桥依赖的 BetterGI 类型、DI 容器和 UI 调度器。", "接口报告成员缺失或版本不兼容时", "每个探测项独立表示可用、缺失或不可达。", "按目标接口所需依赖判断；不把探测成功当作任务成功。", "只读。", "{}"),
            ...
            "bgi.start_game" => ("启动原神", "让宿主按「联动启动」的配置拉起原神并开始截图，随后返回最新状态。", "游戏或截图器没就绪、而用户要求运行游戏任务时（用户说「启动游戏」「打开原神」也是这个意思）；由你调用，不要反过来要求用户自己启动", "started 表示已经发出启动；ready 才是可以跑任务。stillLoading=true 是加载中的过程状态，不是失败。宿主拒绝启动时本接口直接失败并说明缺哪一项，那类错误要把缺项讲给用户。", "用 bgi.get_status 等到 ready=true，再执行原任务。", "只启动进程与截图器；不改变已保存的配置。", "{}"),
            ...
            "bgi.search_settings" => ("查找宿主设置", "按业务词、路径、分区和值类型查找 AllConfig 设置；不搜索配置组、脚本或路线。", "需要把设置需求映射到精确 path 时", "items 包含当前值摘要、写入能力和 valueVersion。", "只对选中的 path 再调用 bgi.get_setting。", "只读。", """{"terms":["拾取"],"limit":20}"""),
            "bgi.get_setting" => ("读取宿主设置", "读取一个精确 path 的当前值、Schema、写入限制和并发版本。", "修改前取最新版本，或提交后回读", "currentValue 是当前内存值；敏感值被遮蔽。", "保存 valueVersion；按 valueSchema 生成新值。", "只读。", """{"path":"triggerInterval"}"""),
            ...
            "bgi.list_commands" => ("宿主命令目录", "列出低层 WPF 命令的目标、参数、可调用状态和影响。命令通常作用于界面当前选择。", "确实需要宿主动作且尚无精确 command ID 时", "目录项不是用户资源，也不证明当前界面选择正确。", "选择一个命令后读取其独立契约。", "只读。", """{"filter":"运行","includeDangerous":false}"""),
            "bgi.invoke_command" => ("执行宿主命令", "执行已确定且当前可调用的宿主命令；异步命令等待处理器返回。", "命令目标、界面上下文和参数都已确认时；运行时审批负责确认", "executed=true 只表示处理器返回，configurationCheckpoint 只保护配置。", "按具体命令契约读取状态或资源；超时后不盲目重发。", "配置可从检查点恢复；文件、通知、进程和游戏副作用通常不能自动撤销。", """{"command":"list_commands 返回的精确 name"}"""),
        ...
        var sideEffects = id switch
        {
            "bgi.commit_settings" or "bgi.set_setting" or "bgi.rollback_settings" =>
                new[] { "修改宿主内存配置并原子更新 User/config.json。" },
            "bgi.invoke_command" =>
                new[] { "影响由具体命令决定；以该命令独立契约为准。" },
            "bgi.start_game" =>
                new[] { "按宿主已配置的安装路径启动原神进程，并启动截图器与实时触发器；宿主自身配置可能同时改注册表关闭原神 HDR。" },
            _ => new[] { "无写入副作用。" },
        };
        return new(
            title,
