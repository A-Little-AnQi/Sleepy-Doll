...
it("is written as markdown and covers every product surface", () => {
  ...
  expect(source).not.toMatch(/BetterGI/);
  render(<HelpPage />);
  ...
  expect(screen.queryByRole("heading", { name: "BetterGI" })).toBeNull();
...
it("opens the matching product surface from in-text links", () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "去打开插件" }));
...
it("jumps from the table of contents to the matching section", () => {
  ...
  expect(document.getElementById("开始")).toBeTruthy();
});
