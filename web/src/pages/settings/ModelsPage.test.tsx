...
it("starts a new model empty instead of picking a provider", () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "添加" }));
  expect(
    screen.getByRole("combobox", { name: "服务商" }).textContent,
  ).toContain("选择服务商");
  expect((screen.getByLabelText("名称") as HTMLInputElement).value).toBe("");
  expect(
    screen.getByRole("combobox", { name: "请求协议" }).textContent,
  ).toContain("选择协议");
...
it("links the selected provider to its developer platform", () => {
  render(<ModelsPage bootstrap={bootstrap} reload={async () => undefined} />);
  fireEvent.click(screen.getByRole("button", { name: "添加" }));
  fireEvent.click(screen.getByRole("combobox", { name: "服务商" }));
  fireEvent.click(screen.getByRole("option", { name: "DeepSeek" }));

  const links = ["开发者平台", "申请密钥"].map((name) =>
    screen.getByRole("link", { name }),
  ) as HTMLAnchorElement[];
  expect(links[0]!.href).toBe("https://platform.deepseek.com/");
  expect(links[1]!.href).toBe("https://platform.deepseek.com/api_keys");
});
...
it("keeps request protocol with the basic fields, not under advanced", () => {
  ...
  expect(
    document.querySelector(".model-advanced-body.is-collapsed"),
  ).toBeTruthy();
...
it("exposes the context window for the selected model", () => {
  ...
  expect((screen.getByLabelText("上下文长度") as HTMLInputElement).value).toBe(
    "200000",
  );
});
