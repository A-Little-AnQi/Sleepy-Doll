use super::{journal::Journal, policy::RuntimeConfig, types::*};
use crate::{
    config::BridgeConfig,
    error::{Error, Result},
    tools::validate,
...
pub struct Invocation<'a> {
    ...
    pub call_id: &'a str,
    pub binding: &'a super::catalog::Capability,
    pub arguments: &'a Value,
    pub resources: Value,
    pub catalog: &'a super::catalog::Catalog,
}
impl Bridge {
    ...
    pub async fn request(
        ...
        let value = super::gateway::read_json(response, cancel).await?;
    ...
    pub async fn wait(
        ...
        loop {
            ...
            if matches!(state, "completed" | "failed" | "cancelled" | "interrupted") {
                ...
                let predicates = serde_json::from_value::<Vec<super::catalog::Predicate>>(
                ...
                if state == "completed" && (a.outcome == "unknown" || !predicates.is_empty()) {
                    ...
                    a.outcome = super::verifier::verify(&predicates, &fresh, &a.instance_id).into();
