...
use super::{artifacts::ArtifactStore, kernel::*};
use crate::{
    error::{Error, Result},
    tools::{RiskLevel, VerificationMode},
};
...
impl MutationVerifier for super::adapter::AdapterClient {
    fn verify(&self, snapshots: &[(ResourceSnapshot, Vec<u8>)], request: Value) -> Result<Value> {
        super::adapter::AdapterClient::verify(self, snapshots, request)
    }
}
...
impl OperationStore {
    ...
    pub fn upsert_resource(&self, resource: &ResourceDescriptor) -> Result<()> {
        self.connection.lock().unwrap().execute(
            ...
            params![resource.id, resource.provider_id, resource.kind, resource.version, serde_json::to_string(resource)?, super::types::now()],
        )?;
        Ok(())
    }
    ...
    pub fn link_artifact(
        ...
        self.connection.lock().unwrap().execute(
            ...
            params![artifact_id,owner_kind,owner_id,purpose,super::types::now()],
        )?;
        Ok(())
    }
    ...
    pub fn transition(&self, operation: &mut Operation, next: OperationState) -> Result<()> {
        ...
        candidate.updated_at = super::types::now();
    ...
    pub fn acquire(&self, scope: &str, operation_id: &str) -> Result<bool> {
        Ok(self.connection.lock().unwrap().execute(
            ...
            params![scope, operation_id, super::types::now()],
        )? == 1)
    }
    ...
    pub fn record_evidence(
        ...
        self.connection.lock().unwrap().execute(
            ...
            params![id, operation_id, kind, payload.to_string(), super::types::now()],
        )?;
        Ok(id)
    }
    ...
    pub fn save_workflow(&self, workflow: &super::workflow::Workflow) -> Result<()> {
        workflow.validate()?;
        self.connection.lock().unwrap().execute(
    ...
    pub fn workflow(&self, id: &str) -> Result<super::workflow::Workflow> {
        let payload: String = self.connection.lock().unwrap().query_row(
    ...
    pub fn workflows(&self) -> Result<Vec<super::workflow::Workflow>> {
        read_payloads(
            &self.connection,
    ...
    pub fn record_workflow_run(
        ...
        self.connection.lock().unwrap().execute(
            ...
            params![workflow_id, revision, run_id, super::types::now()],
        )?;
        Ok(())
    }
    ...
    pub fn save_grant(&self, grant: &super::permissions::TrustGrant) -> Result<()> {
    ...
    pub fn grants(&self) -> Result<Vec<super::permissions::TrustGrant>> {
        read_payloads(
            &self.connection,
...
impl OperationEngine {
    ...
    pub fn configure_verifiers(&self, adapters: &[Arc<super::adapter::AdapterClient>]) {
    ...
    pub fn permission_decision(
        &self,
        operation: &Operation,
        mode: super::permissions::PermissionMode,
        grants: &[super::permissions::TrustGrant],
    ) -> Result<super::permissions::PermissionDecision> {
        ...
        Ok(super::permissions::PermissionEngine::decide(
            mode,
            &super::permissions::PermissionRequest {
...
fn insert_event(db: &Connection, operation_id: &str, kind: &str, payload: &Value) -> Result<()> {
    db.execute(
        ...
        params![operation_id, kind, payload.to_string(), super::types::now()],
    )?;
    Ok(())
}
...
mod tests {
    use super::*;
    use crate::tools::{CompensationMode, ToolEffect};
